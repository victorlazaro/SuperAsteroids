...
public class GraphicsUtils {
    ...
    /**
     * Converts and angle measured in degrees to the equivalent angle measure in radians.
     ...
     */
    public static Rect scaleRectangle(RectF rect, float scale)
    {
        return new Rect(Math.round(rect.left*scale), Math.round(rect.top*scale),
                Math.round(rect.right*scale),Math.round(rect.bottom*scale));
    }
    ...
    public static Rect rectFToRect(RectF objectBounds) {
        return new Rect(Math.round(objectBounds.left), Math.round(objectBounds.top),
                Math.round(objectBounds.right), Math.round(objectBounds.bottom));
    }
